# 执衡项目收官总攻 v7（终版）——完全授权 · 大规模并行 · 判定权分离 · 断点续作

> v7 = v6 + 第三轮双独立复核残留全部处置。核心修订：①盲审职责分体——"Delivery 盲审代理"
> （禁读仓，判投递工件）与"对账代理"（只读仓直查 git/代码/日志，判一致性与增量完备），
> 消除禁读与职责互斥 ②§0.2 会话注册语义拆分（登记验证结果=保守；改基准 URL=挂业主），
> 消除清单自相矛盾 ③R 回执/回归日志**直录+哈希链**（不经转录），堵内容层喂料 ④增量声明
> 由对账代理跑 git log 全量比对，漏一条=REWORK ⑤引用编号修正+P2 批量处置。
...
**向会话注册.json 的"验证记录"字段追加验证结果/失效状态（只追加不改既有字段）**
非保守（禁自行动，挂业主）：enable/disable 系统计划任务、修改任何配置或程序（含冻结资产修补）、
**修改会话注册.json 的既有字段（基准 URL/账号标识/追认状态）**、任何删除、推送目标变更、master 操作
...
（§4 首清单）→ 绝不空转。**业主实时插话永远优先**：收到插话=完成当前原子操作后检查点落盘+
按 §3 取消协议回收子代理+响应。
...
- R_URL：E:\执衡\05_资源\会话注册.json。先验三件：URL 格式合法+桥 open 可达+归属双因子
  （自标识 ping 回执+页面账号标识比对注册基准；注册无基准→首次验证结果**以"验证记录"追加**登记
  并 owner-notice 报备待追认，后续投递以该登记为基准；任一失败=换新会话）
...
- time-box（按现场第一轮实测×3 修正，修正入 journal；**每进入新一轮 REWORK，里程碑 time-box
  ...
  ≤6min；里程碑基准 4h 或"REWORK 轮次数×2h"取大者
...
B. 批次 A 外审按 V07 口径重走：
   ...
   - receive 轮询（≤6min）不计入投递次数（只读）；**R 回执由 receive --out 直录 delivery+命令行
     输出的 SHA256 直录 MANIFEST（不经转录/转贴）**
   - R 逐项裁决。有效裁决=逐项编号+证据指针，或明确"逐项通过"声明+证据绑定；**判定由独立
     盲审代理对照 §9 基线做出（盲审职责与禁读范围见 §5），施工 AI 无自判权**。REWORK 清单按
     §0.1 红线过滤（白名单式：仅"修改施工产物/补充材料/重跑测试"执行；其余挂 owner-notice+
     回函）。REWORK 重投必须重新 create_evidence 绑定新 HEAD-FROZEN；每进入新一轮 REWORK，
     里程碑 time-box 重置（§3）
...
## 5. 判定权分离与业主沟通（v7：盲审职责分体+直录哈希链）
**判定职能分体为两个独立代理，施工 AI 均无自判权：**
- **Delivery 盲审代理**（全新 spawn，**禁读 delivery 目录与本文件之外一切路径**——只判投递工件）：
  对照 §9 基线与 delivery MANIFEST 判 R 裁决有效性/packet 格式/证据完备；输入=delivery 目录
  （确定性枚举：恰好 5 类具名文件 packet/manifest/diff/regression log/裁决回执+MANIFEST——
  多一个文件=拒审"喂料超纲"（主代理清目录至枚举后重开盲审，不扣投递预算）；少一个=判
  "证据不足 REWORK"）。任务书=§9 尾模板（只填 RUNID/HEAD-FROZEN/目录三槽位）。
  挂死（time-box 20min 无响应）=重 spawn（≤2 次），仍挂=里程碑按"未完成报告"诚实收尾，
  禁回退施工 AI 自判
- **对账代理**（全新 spawn，**只读仓库+只跑只读 git 命令**——直读真实仓库不经过主代理导出，
  天然免疫选择性喂料）：职责=①状态一致性核对（PROJECT_STATE.json/STATUS/branch_registry vs
  git 实际）②增量声明全量比对（`git log HEAD-FROZEN..HEAD --name-status` 与增量声明逐条
  比对，漏一条=该里程碑 REWORK）③3 连绿窗口核验（起止哈希+窗口内代码提交扫描）④断言真实性
  抽查（抽 10 条断言读代码与测试核对逻辑对应）。输出=裁定书（绑定被审工件/提交的 SHA256+
  §9 模板版本哈希）。挂死处置同上
- 里程碑关卡：欠账清零/外审闭环/GATE5/总收官——两代理均 APPROVE 过卡
...
1. 全量测试 3 连绿+断言真实性抽查（对账代理）+哨兵绿+state_doctor DRIFT_FREE
2. 外审有效 PASS：R 逐项裁决/声明（Delivery 盲审复验通过）+证据绑定 HEAD-FROZEN+增量声明
...
- **Delivery 盲审任务书模板**（主代理只填三槽位【RUNID】【HEAD-FROZEN 哈希】【delivery 目录】）：
  {"任务":"对照本基线与 delivery MANIFEST，独立判定 §8-2 是否达成","禁读":"delivery 目录与本
  任务书之外一切路径（含任何 git/代码/叙述文件）","输出":"裁定书（绑定 delivery 全部工件 SHA256
  +本基线版本哈希）：APPROVE 或 REWORK 逐项清单"}
- **对账代理任务书模板**（主代理只填三槽位【RUNID】【HEAD-FROZEN 哈希】【worktree 路径】）：
  {"任务":"只读核对 §8-1/3/6：状态一致性、增量声明全量比对（git log HEAD-FROZEN..HEAD）、
  3 连绿窗口哈希、断言真实性抽 10 条","禁写":"一切路径","输出":"裁定书（绑定被审提交/文件
  SHA256+本基线版本哈希）：APPROVE 或 REWORK 逐项清单"}
