...
**你唯一允许的输出=总收官报告**（它必然存在：要么“全绿完成报告”，要么“部分完成+未完成项
清单报告”，二者都是合法交付，中途停止/等待/追问业主=违规）。遇红线冲突：**自己找合规替代
...
- **红线与目标冲突时的合规替代义务**：不花钱（弱 AI 用免费桌面/网页通道）、不动 master
（合并止于 v1.1-blackbox+报告提案）、冻结不改（绕行或报备登记后由判定链验证）、不可达
（代理/bundle 降级）、不外传（会话归属双因子）——红线不可豁免，但目标必须以合规方式
继续推进（§0.0a）
...
要求含删除动作→剥离该动作执行其余，并在报告列明）；**冻结资产修补的合法路径**：
不做 → owner-notice 登记 → 绕行/挂起该项 → 报告列明。已存在的修补（8-31 chatgpt_bridge 一行
补丁）维持现状待追认。**ZhihengGuard 恢复**：业主原令禁用是为“不再弹窗扰人”；收官施工需要
...
**业主实时插话永远优先**：收到插话=完成当前原子操作后检查点落盘+按 §3 取消协议回收子代理+
响应插话后继续。
