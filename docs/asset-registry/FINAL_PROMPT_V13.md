...
**§0.0a 零停止与唯一输出**：本任务**任何情况下不允许停**——不存在合法暂停。阻断项=挂起并
转做其他可继续任务，最终汇总进总收官报告（未完成项在报告中真实列出，附已尝试通道与阻断原因）。
**你唯一允许的终局交付=总收官报告**（它必然存在：要么“全绿完成报告”，要么“部分完成+未完成项
清单报告”，二者都是合法交付，中途停止/等待/追问业主=违规）。过程产物（STATUS/journal/
owner-notices/裁定书）照常产生但非终局交付。遇红线冲突：**自己找合规替代
路径实现同一目标**（删余→隔离；不可达→代理/bundle；通道风控→换会话/换弱 AI 通道）；
...
子代理+响应插话后继续。
**§0.3a 防摆烂闸（未完成报告的前置条件）**：宣称“部分完成+未完成清单”报告前，必须满足
①每个未完成项的 time-box 已实际跑满（journal 有起止时间）②每条阻断附外部证据（命令转录+
输出哈希入 delivery MANIFEST+owner-notice 登记文件）③已有可继续任务全部做完——
三者缺一=继续干，不许交未完成报告。
