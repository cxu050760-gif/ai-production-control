...
Controller \u5df2\u7ecf\u63a5\u7ba1\u2026\u65b0 generation \u63a5\u7ba1: \u8001 generation
\u7acb\u5373\u5931\u53bb Effect Authority\u201d\u3002
...
- `check_execute_right`: \u6267\u884c effect \u524d\u5fc5\u8bc1\u81ea\u5df1\u4ecd\u662f\u5f53\u524d\u5408\u6cd5\u4ee3\u3002
  STALE_GENERATION/LEASE_EXPIRED -> \u62d2\u7edd\u6267\u884c\uff08\u8001\u6743\u5931\u6548\uff09\u3002
...
def check_execute_right(controller_id: str, generation: int,
    ...
    \u8fd4\u56de: {ok: bool, reason: OK|STALE_GENERATION|LEASE_EXPIRED|NO_LEASE, lease: ...}
    """
    now = now or datetime.datetime.now(datetime.timezone.utc)
    cur = load_lease(path)
    if not cur:
